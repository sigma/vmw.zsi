...
The \class{ServiceProxy} class in \ZSI{} uses a \class{WSDL} instance to send
...
of using WSDL.  The class \class{WriteServiceModule} helps to hides these
...
\section{WriteServiceModule}

The \class{WriteServiceModule} class in \module{ZSI.wsdl2python}
transforms WSDL definitions
to remote proxy interfaces, and XML Schema instances to type codes that describe
the data.  The \class{WSDL} (see section 11.4) class and
\module{ZSI.wstools.XMLSchema}
...
Two files are generated by \class{WriteServiceModule}.  The first is a
...
show how \class{WriteServiceModule} is used.
