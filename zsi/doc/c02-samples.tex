...
a \class{Person} instance, either the class and its typecode must be defined
in a module named \module{ComplexTypes} somewhere in the import path,
or the expected return
type must be passed to the \code{Binding.Receive} method.
...
If the instance has a \code{typecode} attribute, then serialization
