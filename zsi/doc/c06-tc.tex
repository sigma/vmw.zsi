...
The name may be either a text string, or a \samp{(uri, localname)} tuple.
If the name (or \code{localname} tuple element) has a colon, the namespace
...
items within a \class{TC.Struct} object will inherit this from the
container.}
...
A choice is a Python two-element \samp{(name, value)} tuple.
...
\section{Apache Datatypes}

The Apache SOAP project, url{http://xml.apache.org/soap/index.html},
has defined two SOAP datatypes in the 
\code{http://xml.apache.org/xml-soap} namespace.
They are \class{Map} and \class{SOAPStruct}.
...
The \code{SOAPStruct} type is similar to a \class{TC.Struct}, except that
it is typed as a \code{SOAPStruct}.
This datatype can usually be processed by using the \code{type} keyword in a
\class{TC.Struct} instance.
A separate Apache \class{SOAPStruct} class is also available; when used,
a \code{SOAPStruct} is either a list of values or a dictionary of name
and value pairs.

\begin{classdesc}{Apache.SOAPStruct}{name\optional{, **keywords}}
...
\begin{tableiii}{l|c|p{30em}}{textrm}{Keyword}{Default}{Description}
\lineiii{\code{aslist}}{\code{0}}{Use a list of values rather than
a dictionary.}
\end{tableiii}
