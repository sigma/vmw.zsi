...
class WriteServiceModule:
    ...
    def write(self, schemaOnly=False, output_dir=".", types=None):
        ...
        if hasSchema:
            ...
            self.write_service_types(f_types, fd, csuffix=csuffix)
            fd.close()
    ...
    def write_service_types(self, f_types, fd, csuffix="_Def"):
        header = '%s \n# %s.py \n# generated by %s \n# \n# \n%s\n\n'\
	...
	fd.write('\n'*2)
        ...
        for schema in self._wa.getSchemaDict().values():
            self.write_dependent_schema(schema, fd, csuffix=csuffix)
...
class ServiceDescription:
	...
	def fromMessage(self, message, name, namespace, style, use, op=None):
                ...
                if p.getType():
                    ...
                    if tp.typecode[0][0:3] == 'ZSI':
                        ...
                        qualifiedtc[0] = self.nsh.getAlias(tns) + \
                                         '.' + qualifiedtc[0][0:idx] + \
                                         self.csuffix + qualifiedtc[0][idx:]
                        l += qualifiedtc
                        defclass = tp.typecode[0][0:]
                        defclass = defclass[0:defclass.find('(')] + self.csuffix + '()'
                        self.typecode += '\n%sself.%s = %s.%s'\
                                         % ( ID2, p.getName(),
		...
		if p.getElement():
			...
			self.typecode += '\n\n%sdef __init__(self, name=None, ns=None):'\
                        ...
                        self.typecode += '\n%s%s.%s.__init__(self, name=None, ns=None)'\
                            ...
                            # for later use in docstring generation
                        self.typeList.append(['literal', '%s.%sLOCAL%s' % (nsp, tp.name, self.csuffix), False])
		    else:
			self.typecode = '\n\nclass %s: ' % (message.getName())
        ...
        def docString(self, allTypeDict, messageName, isInput):
            ...
            for item in self.typeList:
                if item[1]:
                    orig = 'LOCAL%s' % self.csuffix
                    item[1] = item[1].replace(orig, self.csuffix)
                else:
                    item[1] = 'Any'
            alreadyListed = []
	...
	class PartWriter:
            """Generates a string representation of a typecode representing
               a <message><part>
            """
            def __init__(self, aname_func, csuffix="_Def"):
		self.typecode = None
                self.tns = None
		self.name = None
                self.docCode = []
...
class SchemaDescription:
    """Generates classes for all global definitions and declarations in 
       a schema instance.
    """
    def __init__(self, aname_func = None, csuffix="_Def"):
        self.nsh = NamespaceHash()
        self.typeDict = {}
        self.aname_func = aname_func
        self.csuffix = csuffix
	...
	for name, tp in sdict.items():
            ...
            defaultWriter = 'self.__class__.TypeWriter(self.aname_func, self.csuffix)'
	...
	def fromType(self, myType, parentRef):
	    ...
	    tp = myType
            ...
            if tp.isSimpleType():
                self.name = tp.getName() + self.csuffix
		...
		raise WsdlGeneratorError, 'Not supporting  %s' %(tp.getItemTrace())
            ...
            key = "%s.%s%s" % (alias, tp.getName(), self.csuffix)
            if self.typeList:
                self.typeList.sort()
        ...
        def _elementComplexType(self, tp, etp):
            ...
            if etp.getName():
                ...
                self.precede  = '%s' %(etp.getName() + self.csuffix )
        ...
        def _complexTypeComplexContent(self, tp):
            ...
            if not tc:
                ...
                if dt.getDerivation():
                    ...
                    # IRJ
                    self.basector.set('\n%s%s.%s.__init__(self, name=name, ns=ns, **kw)' % (ID4, nsp, dt.getDerivation() + self.csuffix ))
                    if dt.isExtension():
                        self.postpend.set('\n%s# extending....' % ID4)
                ...
                self.classdef.set('\n\n%sclass %s(%s):' \
                                  % (ID1, tp.getName() + self.csuffix, tc))
                self.initdef.set("\n%sdef __init__(self, name = None, ns = None, **kw):" % ID2)
                ...
                if arrayinfo:
                    ...
                    if arrayinfo[2]:
                        ...
                        atype = arrayinfo[1] + self.csuffix
        ...
        def _complexTypeSimpleContent(self, tp):
            ...
            if dt.isSimpleContent() and dt.getTypeclass():
                ...
                self.precede  = '%s%s' % ( dt.getDerivation(), self.csuffix)
                nsp = self.nsh.getAlias(tp.getTargetNamespace())
                self.classdef.set('\n\n%sclass %s(%s):' \
                                  %(ID1, tp.getName() + self.csuffix,
                                    dt.getDerivation() + self.csuffix))
                ...
                self.basector.set('\n\n%s%s.__init__(self, name=name, ns=ns, **kw)' \
                                  %( ID3, dt.getDerivation() + self.csuffix))
            elif hasattr(tp, '_def'):
                raise WsdlGeneratorError, \
            ...
            for e in mg.getContent():
                if e.isDeclaration() and e.isElement():
                    ...
                    if e.isDeclaration() and e.isWildCard():
                        ...
                        occurs = self._calculateOccurance(e)
                        typecodelist += 'self.__class__.%s(name="%s",ns=ns%s), ' % (e.getName() + self.csuffix, e.getName(), occurs)
                        ...
                        if tpc and e.isXSD():
                            ...
                            nsp = self.nsh.getAlias(etp.getTargetNamespace())
                            typeName = '%s.%s' % (nsp, etp.getName()) + self.csuffix
